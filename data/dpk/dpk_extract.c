...
{
	if ( DpkEndianFlag ) {
		DpkHeader.block_size = CM_ByteSwapU32( DpkHeader.block_size );
		DpkHeader.file_count = CM_ByteSwapU32( DpkHeader.file_count );
		DpkHeader.entry_end  = CM_ByteSwapU32( DpkHeader.entry_end );
		DpkHeader.entry_size = CM_ByteSwapU32( DpkHeader.entry_size );
	}
}
...
{
	if ( DpkEndianFlag ) {
		DpkEntry.offset   = CM_ByteSwapU32( DpkEntry.offset );
		DpkEntry.length   = CM_ByteSwapU32( DpkEntry.length );
		DpkEntry.checksum = CM_ByteSwapU32( DpkEntry.checksum );
	}
}
